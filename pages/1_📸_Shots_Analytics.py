...
if not pop_df.empty:
    ...
    if len(found_in_popular) > 0:
        ...
        **🚀 Як раскачати шот, який потрапив в Popular:**
        - 💬 **Відповідайте на коментарі** — engagement підвищує позицію в Popular, кожна відповідь = +1 comment
        - 🔄 **Запустіть повторний бустинг** — через 2-3 дні після першого, щоб укріпити позицію і потрапити в тижневі/місячні рейтинги
        - 📌 **Pin shot** на профілі VALMAX — нові відвідувачі побачать його першим
        - 📣 **Поширити** — Instagram Stories, Telegram, Twitter, LinkedIn
